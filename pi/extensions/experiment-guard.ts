...
// The bash tool's default timeout comes from the @cad0p/pi-bash-timeout package
// pinned in the image and loaded through the frozen settings (120 s when the
// agent sets none). The guard only records the commands it cuts off.
...
export default function experimentGuard(pi: ExtensionAPI) {
  ...
  // A command the default timeout cut off returns an error result whose text
  // ends with Pi's timeout status line; record it so reports can count hangs
  // prevented.
