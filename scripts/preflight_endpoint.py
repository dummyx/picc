...
server for its health and metadata (`/health` and `/props`) and compares its
model alias with `MODEL_ID`. Exit status: 0 the alias matches (or nothing to
verify for a hosted provider), 2 the alias does not match, 3 the endpoint is
unreachable or does not expose its model alias.
...
def preflight(config: dict[str, str], *, timeout: float) -> tuple[int, dict[str, Any]]:
    ...
    if not served_alias:
        ...
        return 3, record
    if served_alias != expected_model:
        record["status"] = "mismatch"
        record["problems"] = [f"served model alias {served_alias!r} is not MODEL_ID {expected_model!r}"]
        return 2, record
