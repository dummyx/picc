#!/usr/bin/env python3
"""v12 cohort: per-replicate hidden scores for the chapters 1-18 compiler and
...
paired deltas.  Read-only over runs/study-results/.
    ...
    python3 analysis/v12_results.py
"""
...
STUDIES = {"picc-c18-minimal-v3": "chapters 1-18 compiler", "picc-sql-minimal-v3": "SQL engine"}
FLOORS = {"picc-c18-minimal-v3": 0.2129, "picc-sql-minimal-v3": 0.0503}
...
def rescored() -> dict[str, dict[str, float]]:
    ...
    path = ROOT / "analysis" / "v12-rescore" / "summary.json"
    if not path.is_file():
        return {}
    return {
...
def main() -> int:
    ...
    (ROOT / "analysis" / "v12-results.json").write_text(json.dumps(output, indent=2) + "\n", encoding="utf-8")
    return 0
