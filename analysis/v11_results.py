...
paired deltas.  Read-only over runs/study-results/.
    ...
    python3 analysis/v11_results.py
"""
...
STUDIES = {"picc-c18-minimal-v2": "chapters 1-18 compiler", "picc-sql-minimal-v2": "SQL engine"}
FLOORS = {"picc-c18-minimal-v2": 0.2129, "picc-sql-minimal-v2": 0.0503}
...
def rescored() -> dict[str, dict[str, float]]:
    ...
    path = ROOT / "analysis" / "v11-rescore" / "summary.json"
    if not path.is_file():
        return {}
    return {
