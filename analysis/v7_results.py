...
Writes analysis/v7-results.md, analysis/v7-results.json (and a compact
analysis/v7-rescore.json when a re-score exists).

Usage:
    python3 analysis/v7_results.py [--rescore-dir DIR]
"""
...
OUT = REPO / "analysis"
RESCORE = Path("/tmp/claude-1003/-home-xie-picc/8b7aadf0-3d32-4755-86ac-4fecb9f08861/scratchpad/v7-rescore")
sys.path.insert(0, str(OUT))
import dimensions  # noqa: E402  (analysis/dimensions.py: tokens, time, code shape)

BASELINE, VARIANT = "js-untyped", "ts-strict"
IDS = [f"v7-{cond}-r{rep}" for rep in (1, 2, 3, 4) for cond in (BASELINE, VARIANT)]
V4_IDS = [f"v4-{cond}-r{rep}" for rep in (1, 2, 3) for cond in (BASELINE, VARIANT)]
THRESHOLD = 0.13
SECONDARY = [
    ...
    ("test_visible_calls", "test_visible calls"), ("tsc_calls", "tsc invocations"), ("tsc_blocked", "tsc attempts blocked"), ("source_loc", "source LOC"), ("functions", "functions"),
...
def run_row(rid: str) -> dict | None:
    ...
    return {
        "run_id": rid,
        "condition": VARIANT if VARIANT in rid else BASELINE,
        ...
        "tsc_calls": tsc_invocations(run),
        "tsc_blocked": sum(1 for r in guard_rows if r.get("event", "blocked_tool_call") == "blocked_tool_call" and "type check" in str(r.get("reason", ""))),
...
def tsc_invocations(run: Path) -> int:
    """Bash tool calls whose command runs tsc (treatment fidelity: the typed arm
    should invoke the checker; the untyped arm's attempts are guard-blocked)."""
    import re
    count = 0
    for path in sorted((run / "artifacts" / "events").glob("round-*.jsonl")):
        for e in dimensions.load_jsonl(path):
            if e.get("type") == "tool_execution_start" and e.get("toolName") == "bash":
                if re.search(r"(?:^|[\s;&|/])tsc\b", str((e.get("args") or {}).get("command", ""))):
                    count += 1
    return count
...
def main() -> None:
    ...
    with contextlib.redirect_stdout(buffer):
        rows = report_rows(IDS, "v7")
        print("=== v4 for the harness check (same contrast on image 0.2, original oracle, no default timeout; never pooled with v7)")
        v5 = report_rows(V4_IDS, "v4")
    text = buffer.getvalue()
    print(text)
    (OUT / "v7-results.md").write_text("# v7 results digest\n\n```\n" + text + "```\n", encoding="utf-8")
    (OUT / "v7-results.json").write_text(json.dumps({"v7": rows, "v4": v5}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    if compact:
        (OUT / "v7-rescore.json").write_text(json.dumps(compact, indent=2, sort_keys=True) + "\n", encoding="utf-8")
